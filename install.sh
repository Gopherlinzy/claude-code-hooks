...
# 跨平台文件大小（macOS BSD stat → Linux GNU stat → fallback wc）
file_size() {
...
# Mac/Linux sed 写法差异：Mac (BSD) 用 -i'' 默认，Linux (GNU) 用 -i 或 -i.bak
sed_inplace() {
...
_on_error_handler() {
  ...
  if [ -n "${BACKUP_FILE:-}" ] && [ -f "$BACKUP_FILE" ]; then
    ...
    # 更可靠的终端检测：检查 fd3 或 stdin，而不只是 true <&3
    if [ "$NON_INTERACTIVE" = false ] && test -t 3 2>/dev/null; then
...
run_install() {
  ...
  if [ "$SKIP_CONF" = false ]; then
    ...
    # 写入 conf 文件：先原子创建 mode 600，避免时间窗口
    local CHANNEL_UPPER
    CHANNEL_UPPER="$(echo "$CC_CHANNEL" | tr '[:lower:]' '[:upper:]')"

    # 原子创建 600 权限的文件
    install -m 600 /dev/null "$CONF_FILE" 2>/dev/null || {
